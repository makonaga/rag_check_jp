...
class HuggingFaceTextEmbeddingModel:
    ...
    def __init__(self, pretrained_model_name: str = 'intfloat/multilingual-e5-large', gpu_id: int = 0):
...
if __name__ == "__main__":
    ...
    model = HuggingFaceTextEmbeddingModel("intfloat/multilingual-e5-large", 0)
    text1 = "これは日本語のテスト文章です。"
    text2 = "今日は良い天気です。"
    embedding1 = torch.tensor(model.get_embedding(text1))
    embedding2 = torch.tensor(model.get_embedding(text2))
    from torch.nn.functional import cosine_similarity
